...
function ensureRequiredPackages(context, yes) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const confirmInstall = (pkg, requiredVersion, opts) => __awaiter(this, void 0, void 0, function* () {
            let success = false;
            const command = npm_utils_1.installCommand(pkg, opts);
            ...
            if (upgrade) {
                success = npm_utils_1.installUpgrade(pkg, opts);
            }
            else {
                success = false;
            }
        ...
        if (!cliVersion || semver.gt(requireds["@plasmicapp/cli"], cliVersion)) {
            ...
            throw new error_1.HandledError(chalk_1.default.bold(`@plasmicapp/cli has been upgraded; please try again`));
        }
...
function writeLoaderConfig(opts, config) {
    ...
    file_utils_1.writeFileText(loaderConfigPath, code_utils_1.formatAsLocal(JSON.stringify(config), loaderConfigPath));
}
/**
...
function sync(opts, metadataDefaults) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!opts.skipUpgradeCheck) {
            yield ensureRequiredPackages(context, opts.yes);
        }
        ...
        yield file_utils_1.withBufferedFs(() => __awaiter(this, void 0, void 0, function* () {
            ...
            for (const m of pendingMerge) {
                const resolvedEditedFile = code_utils_1.replaceImports(context, m.editedSkeletonFile, m.skeletonModulePath, fixImportContext, true);
                const resolvedNewFile = code_utils_1.replaceImports(context, m.newSkeletonFile, m.skeletonModulePath, fixImportContext, true);
            ...
            yield code_utils_1.fixAllImportStatements(context, summary);
            ...
            if (process.env.PLASMIC_LOADER) {
                ...
                const freshIdsAndTokens = projectIdsAndTokens
                    ...
                    .map((p) => lodash_1.default.pick(p, "projectId", "projectApiToken"));
                const config = {
                    projects: lodash_1.default.sortBy(lodash_1.default.uniqBy([...freshIdsAndTokens, ...((_b = loaderConfig === null || loaderConfig === void 0 ? void 0 : loaderConfig.projects) !== null && _b !== void 0 ? _b : [])], (p) => p.projectId), (p) => p.projectId),
                };
...
function syncProject(context, opts, projectIdsAndTokens, projectId, componentIds, projectVersion, dependencies, summary, pendingMerge, metadataDefaults) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const projectApiToken = lang_utils_1.ensure(projectIdsAndTokens.find((p) => p.projectId === projectId)).projectApiToken;
        const existingChecksums = checksum_1.getChecksums(context, opts, projectId, componentIds);
        // Server-side code-gen
        const projectBundle = yield context.api.projectComponents(projectId, context.config.platform, newComponentScheme, existingCompScheme, componentIds, projectVersion, context.config.images, context.config.style, existingChecksums, get_context_1.generateMetadata(Object.assign(Object.assign({}, metadataDefaults), { platform: context.config.platform }), opts.metadata));
        // Convert from TSX => JSX
        if (context.config.code.lang === "js") {
            projectBundle.components.forEach((c) => {
                [c.renderModuleFileName, c.renderModule] = code_utils_1.maybeConvertTsxToJsx(c.renderModuleFileName, c.renderModule);
                [c.skeletonModuleFileName, c.skeletonModule] = code_utils_1.maybeConvertTsxToJsx(c.skeletonModuleFileName, c.skeletonModule);
            });
            projectBundle.iconAssets.forEach((icon) => {
                [icon.fileName, icon.module] = code_utils_1.maybeConvertTsxToJsx(icon.fileName, icon.module);
            });
            projectBundle.globalVariants.forEach((gv) => {
                [gv.contextFileName, gv.contextModule] = code_utils_1.maybeConvertTsxToJsx(gv.contextFileName, gv.contextModule);
            });
            (projectBundle.projectConfig.jsBundleThemes || []).forEach((theme) => {
                [theme.themeFileName, theme.themeModule] = code_utils_1.maybeConvertTsxToJsx(theme.themeFileName, theme.themeModule);
            });
        }
        yield sync_global_variants_1.syncGlobalVariants(context, projectBundle.projectConfig, projectBundle.globalVariants, projectBundle.checksums);
        syncCodeComponentsMeta(context, projectId, projectBundle.codeComponentMetas);
        yield syncProjectConfig(context, projectBundle.projectConfig, projectApiToken, projectVersion, dependencies, projectBundle.components, opts.forceOverwrite, !!opts.appendJsxOnMissingBase, summary, pendingMerge, projectBundle.checksums);
        yield sync_styles_1.upsertStyleTokens(context, projectBundle.usedTokens);
        yield sync_icons_1.syncProjectIconAssets(context, projectId, projectVersion, projectBundle.iconAssets, projectBundle.checksums);
...
function syncProjectConfig(context, projectBundle, projectApiToken, version, dependencies, componentBundles, forceOverwrite, appendJsxOnMissingBase, summary, pendingMerge, checksums) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (projectBundle.cssRules) {
            const formattedCssRules = code_utils_1.formatAsLocal(projectBundle.cssRules, projectConfig.cssFilePath);
        ...
        */
        ...
        yield sync_components_1.syncProjectComponents(context, projectConfig, version, componentBundles, forceOverwrite, appendJsxOnMissingBase, summary, pendingMerge, projectLock, checksums);
    });
}
